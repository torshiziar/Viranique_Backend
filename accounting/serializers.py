...
from accounting.validators import update_username_validator, create_username_validator, phone_validator, email_validator, photo_validator,phone_validator_edit
import logging
import random
import string
...
class UpdateSubUserSerializer(serializers.ModelSerializer):
    ...
    #== New Code ===
    phone = serializers.CharField(required=False, validators=[phone_validator_edit])
    #===
...
class AddSubUserSerializer(serializers.ModelSerializer):
    ...
    phone = serializers.CharField(max_length=50, min_length=11, validators=[phone_validator])
    
    # #==== New Code ====
    # password=serializers.CharField(max_length=32, allow_null=False, allow_blank=False)
    # #========

    class Meta:
        model = User
        # fields = ['name', 'last_name', 'email', 'username', 'phone', 'is_admin', 'can_monitor', 'can_control','password']
