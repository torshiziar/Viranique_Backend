...
class RemainAccountView(mixins.RetrieveModelMixin, mixins.UpdateModelMixin, generics.GenericAPIView):
    queryset = User.objects.all()
    
    # def get(self, request, username):
    #     user = get_object_or_404(User, username=username)
    #     remain_account = user.remain_account
    #     return Response({'remain_account': remain_account})
    
    def get(self, request, *args, **kwargs):
        user = self.queryset.get(username=request.user.username)
        remain_account = user.remain_account
        # serializer = self.serializer_class(instance=user)
        return Response({'remain_account': remain_account},status=status.HTTP_200_OK)
