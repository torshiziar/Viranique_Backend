...
def send_automatic_command(self, actuator_id, organization_id, topic, value, automation, plan):
    ...
    if instance.exists():
        ...
        if self.max_retries > self.request.retries > 0:
            ...
            # logger.debug("============== FIND USERs (FAILED)==============="+str(len(users)))
            # send_status_sms.delay(users, "FAILED", plan, organization_id)
        ...
        # logger.debug("NOTTTTT instance.exists()================")
        ...
        # #==== NEWW =====
        # users = User.objects.filter(organization_id=organization_id)
        # logger.debug("============== FIND USERs (SUCCESS)==============="+str(len(users)))
        # send_status_sms.delay(users, "SUCCEED", plan, organization_id)
        # #=======
...
def send_status_sms(self, users, status, plan, organization_id):
    logger.debug("====================Check Status 0= ======================")
    message = ""
    logger.debug("====================Check Status 1= ======================")
    organ = Organization.objects.get(id=organization_id)
    logger.debug("====================Check Status 2= ======================")
    organ_users = organ.users.all()
    logger.debug("====================Check Status 3= ======================")
    plan_obj = ManualTile.objects.get(id=plan["id"])
    logger.debug("====================Check Status 4= ======================")
    if status == "SUCCEED":
        logger.debug("====================SUCCEED 1=======================")
        if plan["active"]:
            logger.debug("====================SUCCEED 2=======================")
    ...
    if organ.message:
        ...
        logger.debug("READY TO SEND_STATUS_SMS")
        ...
        try:
            ...
            sms = api.sms()
            to = phones
            logger.debug("Phone ============ "+str(to))


            #== for Multiple Phone numbers ===
            lst_phones=list()
            if ('-' in to[0]):
                lst_phones=to[0].split('-')
                logger.debug("Multiple_Send_SMS")


            else:
                lst_phones.append(to)
            #========
            
            for phone_one in lst_phones:
                _from = '50004001927031'
                text = sms_message
                response = sms.send(phone_one,_from,text)


        except Exception as ee:
            logger.debug("ERROR STATUS_SENDD=== "+str(ee))

        #===============================

        logger.debug("STATUS_SENDD=== "+str(response.status_code) +"   des== " + str(response.text))
...
def automation_update_manual_status(self, actuator_id, organization_id, value):
    try:
        ...
        if value == 1.0:
            ...
            active = True
        if qs.exists():
            # logger.debug("QS.EXISTS(==========)000000000")
            automation = qs.first()
            ...
            #==== NEWW =====????????????????????============
            logger.debug("QS.EXISTS(==========)111111111")
            users = User.objects.filter(organization_id=organization_id)
            logger.debug("QS.EXISTS(==========)2222222222")
            users_dic = []
            if users.exists():
                user=users.first()
                logger.debug("QS.EXISTS(==========)33333333333")
                users_dic = [{"id": user.id, "phone": user.phone, "email": user.email, "automation": plan["title"]}]
                logger.debug("QS.EXISTS(==========)4444444444")
                            
            logger.debug("============== FIND USERs (SUCCESS)==============="+str(len(users)))
            send_status_sms.delay(users_dic, "SUCCEED", plan, organization_id)
