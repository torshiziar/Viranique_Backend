    ...
    async def stream(self, tab_id):
        ...
        while True:
            for tile in self.selected_tab.tile_set.all():
                ...
                precision = precision_dict.get(tile.precision, 0)
...
#######################################################################################################################
    ...
    async def stream_new(self, event):
        ...
        logger.debug('===============Start stream_new ================')


        if self.selected_tab:

            # logger.debug('self.selected_tab ==== '+str(self.selected_tab))

            for _ in self.selected_tab.tile_set.all():
                try:

                    logger.debug('_.id ==== '+str(_.id))
                    ...
                    if tile.sensor.id == event["sensor_id"]:
                        ...
                        logger.debug('sensor_id  =========== '+str(event["sensor_id"]))
                        ...
                        logger.debug('Update_tile.id ==== '+str(tile.id))
                        


                except Exception as e:
                    logger.debug('---------ERRRRRRORRRRRRRRR----------')
    ...
    #         if (len(all_sensor_DF)>0):
    #             await self.send(text_data=json.dumps([df.to_dict() for df in all_sensor_DF]))
    #             await asyncio.sleep(1)
