...
class MyMqttConsumer(MqttConsumer):
    ...
    def update_sensor(self, topic, value):
        ...
        with transaction.atomic():
            async_to_sync(self.delayy_random)
            
            lstWill=list()
            sensor_by_topic = Sensor.objects.filter(value_topic=topic)
            if sensor_by_topic.exists():
                will_topicc=sensor_by_topic.first().will_topic
                qs = Sensor.objects.filter(will_topic=will_topicc)
                # qs = Sensor.objects.prefetch_related('organization__users').filter(will_topic=will_topicc,is_online=True) # agar hamegi offline bashand
                # logger.debug(will_topicc+"  len_All_willTopic ==== "str(will_topicc)+" ==== "+str(len(qs)))

                for sensorWill in qs:
                    # logger.debug("====CHECK ReSULT==== "+str(sensorWill.will_topic))

                    if (sensorWill.is_online==True):
                        # noWill=1
                        lstWill.append(1)
                        break

        # logger.debug("====CHECK ReSULT==== "+str(noWill))
        ...
        with transaction.atomic():
            ...
            if sensor_by_topic.exists():
                ...
                # qs = Sensor.objects.filter(will_topic=will_topicc)# agar hamegi offline bashand
                # qs = Sensor.objects.prefetch_related('organization__users').filter(will_topic=will_topicc, is_online=True)

                
                # noWill=0

                # logger.debug("====Check IS ONLINE SENSOR===="+str(len(qs)))
                # if (len(qs)==0):
                ...
                # logger.debug("====CHECK ReSULT==== "+str(len(qs)))
                # if (noWill==0):
                if (len(lstWill)==0):
                    logger.debug("====CHECK ReSULT==== "+" Send Notifffff")
                    sensor=sensor_by_topic.first()
                ...
                for sensor in sensor_by_topic:
                    logger.debug("====CHECK ReSULT==== "+" No NOTIFFF")
                    sensor.value = float(value)
                    sensor.is_online = True
                    sensor.is_connected = True
    ...
    #                 # try:
    #                 #     float(float(value))
    #                 #     logger.debug("IN device.Consumer === "+ str(float(float(value))))
    #                 # except:
    #                 #     cont=0
    #                 # if (cont==0):
    #                 #     continue
    ...
    #                 channel_layer = channels.layers.get_channel_layer()
    #                 async_to_sync(channel_layer.group_send)("Monitoring", {"type": "stream_new", "sensor_id": sensor.id})
    #                 logger.debug("sensor_by_topic-------- stream_new "+" ----  sensor.id== "+str(sensor.value_topic))
    ...
    #         #***************************************************************************
    #         #=====================
    #         # in ghesmat ERROR darad!! ---> value_Topic eshtebah ijad mikonad ---> sababe Error-> Delete All Sensors and tiles mishavad!!!
    #         # #==============================      

    #         # else:
    #         #     logger.debug("sensor_by_topic-------- Not Exists!!!!"+" ----  value_topic== "+str(topic))
