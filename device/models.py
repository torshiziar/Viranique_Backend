...
class Sensor(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        #     logger.debug("IN device.Consumer === "+ str(float(float(self.value))))
        # except:
        #     cont=0

        # if (cont==1):
        # #===========
        logger.debug("Sensor saveeee starteddd!!!!  "+str(self.value_topic)+"  "+str(self.value))
        if self.id is not None:
            if self.tracker.has_changed('value'):
                self.previous_value = self.tracker.previous('value')
                self.updated_at = timezone.now()
            ...
            if self.tracker.has_changed('is_online'):
                if self.is_online:
                    for user in self.organization.users.all():
                        notify.send(sender=self, recipient=user, verb='has connected', target=self.organization)
                        # text_to_present=f"اینترنت همه سنسورهای {} وصل شد"
                        # notify.send(sender=self, recipient=user, verb='همه سنسورهای نود ', target=self.organization)
                        
                        # Check shavad ke hatman "user" va "self.organization" hatman dorost bashad!!!
                        # gahi vaght ha yek sensor baraye 2 user tarif shodand ke baes mishavad ke norification barash nayayad!!!!!!
                        # notification faghat baraue user1 miravad va user2 nemigirad!!! (choon organizatio motefaveti dashtand!!!)
                        logger.debug("notify Senddd!!!!---- Connect " +str(user)+"  "+str(self.organization)) 

                else:
                    logger.debug("Sensor Change Online!!!!")
                    for user in self.organization.users.all():
                        notify.send(sender=self, recipient=user, verb='has disconnected', target=self.organization)
                        logger.debug("notify Senddd!!!!---- Disconnect")
                self.updated_at = timezone.now()
        super(Sensor, self).save(*args, **kwargs)

        logger.debug("Sensor saveeee Enddedddd!!!!  "+str(self.value_topic)+"  "+str(self.value))
