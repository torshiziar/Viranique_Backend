...
def sensor_signal(sender, instance, **kwargs):
    ...
    if not qs.exists():
        logger.debug("111 qs not Exists -----------")
        return
    if instance.value is None:
        logger.debug("isntance value in NONE ------")
        return
    ...
    #==================NEW (REZVANI) Log details ==========
    # dar inja baraye SMS zadan --> within_interval ra hazf kardim!!! 
    # yani inke be mahze inke taighirat dar sensor rokh dahad --> SMS miayad!!! (time interval bardashte shode ast!!!!)

    logger.debug("action_object_id=== "+str(instance.id)+
                 "  action_object_content_type==== "+str(device_ctype)+
                 "  instance.value=== "+str(instance.value))
    qs = NotificationManager.objects.filter(action_object_content_type=device_ctype, action_object_id=instance.id).filter(
        Q(condition='HT', set_point__lt=instance.value) | Q(condition='LT', set_point__gt=instance.value))
    if (qs.exists()):
        logger.debug(" Sensorrrr is Existsss!")
    ...
    if not qs.exists():
        logger.debug("222 qs not Exists -----------")
        return
    users = User.objects.filter(organization_id=instance.organization_id)
    try:
        if users.exists():
